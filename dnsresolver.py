...
def send_dns_query(server, domain):
    ...
    try:
        ...
        return dns.query.udp(query,server,TIMEOUT)
        #Sends the dns query over udp protocol to the DNS server whose IP address is given by "server"
...
def extract_next_nameservers(response):
    ...
    for rrset in response.authority:
        # Check if the record set is of type NS (nameserver record)
        if rrset.rdtype == dns.rdatatype.NS:
            # Loop through each record within this NS record set.
            for rr in rrset:
                # Convert the NS record to text to extract the nameserver hostname.
                ns_name = rr.to_text()
                # Append the extracted nameserver hostname to the ns_names list.
                ns_names.append(ns_name)
                # Print a debug message indicating that an NS hostname has been extracted.
                print(f"Extracted NS hostname: {ns_name}")
    # Loop through each nameserver hostname stored in ns_names.
    for ns_name in ns_names:
        try:
          # Resolve the nameserver hostname by querying for its A record (IPv4 address)
          # and loop through each answer and convert and store the IP addresses
...
def iterative_dns_lookup(domain):
    ...
    while next_ns_list:
        ...
        if response: #checks if response is not NONE
            ...
            #If current stage is root move to TLD in the hierarchy and then to AUTH to finally get the domain
            if stage == "ROOT":
                stage = "TLD"
            elif stage == "TLD":
                stage = "AUTH"
...
def recursive_dns_lookup(domain):
    ...
    try:
        # Perform recursive resolution using the system resolver
        answer = dns.resolver.resolve(domain, "A", lifetime=TIMEOUT)
        # lifetime specifies the time interval for which the query should wait, till timeout occurs.

        # Extract authoritative names if available (assuming NS information exists)
        ns_answers = dns.resolver.resolve(domain, "NS")
        for ns_rdata in ns_answers:      # Printing the authoritative server names
            print(f"[SUCCESS] {domain} -> {ns_rdata}")

        # Finally, print resolved A record
        for rdata in answer:
            print(f"[SUCCESS] {domain} -> {rdata}")
    except dns.resolver.NXDOMAIN:          # In case domain name is not found
        print(f"[ERROR] Domain {domain} does not exist.")
    except dns.resolver.NoAnswer:      # In case no answer is recieved from the resolver
        print(f"[ERROR] No answer for the query.")
    except dns.resolver.Timeout:    # In case of time out situation
